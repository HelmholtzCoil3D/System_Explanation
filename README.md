...
For example there's a script that find the currents necessary to have specificaly field and save it in a CSV file, later this data can be verify in another script that opens this CSV file and plot the graphics and then another scrip to apply the desired currents. Following the graphic generated by [system_plot.py] from a rotating x,y field found by [system_current_finder.py] as an example:
